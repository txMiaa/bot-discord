...
4️⃣ → +26`);
    ...
    for (let e of ["1️⃣","2️⃣","3️⃣","4️⃣"]) {
      try { await idadeMsg.react(e); } catch (err) { console.log(err); }
    }
...
🕊️ Presbiteriano`);
    ...
    for (let e of ["📖","🔥","📜","⛪","✝️","🕊️"]) {
      try { await vertenteMsg.react(e); } catch (err) { console.log(err); }
    }
...
🖤 Preto`);
    ...
    for (let e of ["💗","💚","💙","❤️","🤍","🖤"]) {
      try { await corMsg.react(e); } catch (err) { console.log(err); }
    }
